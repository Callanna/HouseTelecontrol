package com.github.callanna.housetelecontrol.data;
...
public class ImageManager {
    ...
    public VideoList allImages(Context ctx, ContentResolver cr, int inclusion, int sort) {
        if (cr == null) {
            return null;
        } else {
    ...
    // zip source bitmap to proper size and transfer it to byte[]
    static public byte[] miniThumbData(Bitmap source) {
        if (source == null) {
            return null;
        }
        // Zip Bitmap to thumbnail size
        Bitmap miniThumbnail = extractMiniThumb(source, MINI_THUMB_TARGET_WIDTH,
                MINI_THUMB_TARGET_HEIGHT);
